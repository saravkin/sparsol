function [x,r,g,info] = pqn_mmv( A, B, sigma, options )
%PQN_MMV  Solve multi-measurement basis pursuit denoise (BPDN)
%
%   PQN_MMV is designed to solve the basis pursuit denoise problem
%
...
%                                  v = A'*w  if mode == 2. 
%   
%   X = PQN_MMV(A,B,SIGMA) solves the BPDN problem.  If SIGMA=0 or
%   SIGMA=[], then the basis pursuit (BP) problem is solved; i.e., the
%   constraints in the BPDN problem are taken as AX=B.
%
%   X = PQN_MMV(A,B,SIGMA,OPTIONS) specifies options that are set using
%   SPGSETPARMS.
%
%   [X,R,G,INFO] = PQN_BPDN(A,B,SIGMA,OPTIONS) additionally returns the
...
options.project     = @(x,tau) NormL12_project(groups,x,weight,tau);
options.primal_norm = @(x    ) NormL12_primal(groups,x,weight);
options.dual_norm   = @(x    ) NormL12_dual(groups,x,weight);

% Call solver
[x,info] = pqnSolve(A,B(:),[],sigma,[],options);
