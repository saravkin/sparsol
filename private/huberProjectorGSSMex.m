function [x, itn] = huberProjectorGSSMex(b,d,tau, eps)
...
%%
   ...
   % Set number of iterations - not used
   itn = 0;
end
...
function [differ x] = huberDifferTau(alpha, b, eps, tau)
x = (b - alpha).*(b > (alpha + eps)) + (b <= (alpha + eps)).*eps.*b/(alpha + eps);
differ = abs(hubers(x, eps) - tau);
